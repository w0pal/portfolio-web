...
export default function BlogDetailContent({ post }: BlogDetailContentProps) {
 ...
 const cleanContent = post.fullContent
  .replace(/<figure[^>]*>/gi, '<figure class="my-6 p-4 rounded-lg bg-gray-100 dark:bg-slate-700">')
  .replace(/<img[^>]*>/gi, '<div class="text-center text-sm opacity-60 mb-2">[Gambar - lihat di Medium]</div>')
  .replace(/<figcaption[^>]*>/gi, '<figcaption class="text-center text-sm opacity-80 italic">')
